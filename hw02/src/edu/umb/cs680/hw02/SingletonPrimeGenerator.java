...
public class SingletonPrimeGenerator {
    ...
    public static SingletonPrimeGenerator getInstance(long newFrom, long newTo) {
        if (instance == null) {
            instance = new SingletonPrimeGenerator(newFrom, newTo);
        } else if ((newFrom >= 1 && newTo > from)) {
                from = newFrom;
                to = newTo;
        } else {
                throw new RuntimeException("Wrong input values: from=" + newFrom + " to=" + newTo);
        }
